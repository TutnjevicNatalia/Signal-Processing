...
%   RF_NUM_TREES, RF_MAX_DEPTH, RF_MIN_LEAF
    ...
    switch CLASSIFIER_TYPE_L
        ...
        case {'random_forest','rf','randomforest'}
            ...
            % % --- class weighting (balanced) --- Make cost sensitive
            % % instead
            % classes = unique(y);
            ...
            % --- cost-sensitive class weighting (balanced) ---
            classes = unique(y);
            counts  = arrayfun(@(c)sum(y==c), classes);
            
            % Strength of cost-sensitivity (tunable)
            alpha = 1.0;  % 0=no weighting, 1=inverse frequency, >1 stronger
            try, alpha = evalin('base','RF_CLASS_WEIGHT_ALPHA'); end
            
            w_per_class = (1 ./ counts) .^ alpha;
            
            % Optional: normalize so mean weight ~ 1 (numerically nicer)
            w_per_class = w_per_class / mean(w_per_class);
