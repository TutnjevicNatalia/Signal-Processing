...
% Always define feature_names (used from Iteration 2+)
feature_names = [];
...
if CURRENT_ITERATION == 1
    ...
    % --- Subject-wise normalisation BEFORE global scaling (Iter 3+) ---
    if CURRENT_ITERATION >= 3 && isfield(channel_info, 'subject_ids') ...
                              && ~isempty(channel_info.subject_ids)
        fprintf('Applying subject-wise z-scoring of features (Iteration %d).\n', CURRENT_ITERATION);
...
if CURRENT_ITERATION == 2
    K_TARGET = 40; 
    [selected_features, sel_idx] = select_features(features, labels, feature_names, K_TARGET);
...
if ~isempty(feature_names) && numel(feature_names) == size(features,2)
    ...
    for k = 1:numel(sel_idx)
        fprintf('  %2d) idx=%3d  %s\n', k, sel_idx(k), feature_names{sel_idx(k)});
    end
else
    fprintf('\n(No valid feature_names available – only indices printed.)\n');
end
