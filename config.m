...
% -- File Paths --
DATA_DIR = 'C:\Users\ester\Documents\Signal processing\Data\Training';
TRAINING_DIR = DATA_DIR;
HOLDOUT_DIR = DATA_DIR;
SAMPLE_DIR = DATA_DIR;
CACHE_DIR = 'cache/';
...
[preprocessed_data, labels] = preprocess(DATA_DIR);

% -- Feature Extraction --
% (Add feature-specific parameters here)
features = feature_extraction_extract_features(preprocessed_data, CURRENT_ITERATION);
% Print first 5 epochs and their features
%disp('First 5 epochs, features:');
%disp(features(1:5, :));
%min(preprocessed_data(:))
%max(preprocessed_data(:))
%mean(preprocessed_data(:))
